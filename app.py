...
def init_db():
    ...
    # 3. Products & High-Value Deals Table
    c.execute('''
    ...
    # 5. Wallet Recharges & Withdrawals History Table
    c.execute('''
    ...
    # 6. Site Visit Bookings Table (Real Estate / Auto)
    c.execute('''
    ...
    if c.fetchone()[0] == 0:
        c.execute('''
            ...
            VALUES 
            ('Mamta General & Cloth Store', '919876543210', 'mamtastore@upi', 'Nagpur', 'Main Market', '27ABCDE1234F1Z5', 'N/A', 1, 1, 21.1458, 79.0882, 4.9, 150.0, 1, 20.0, 30.0, 10.0),
        ...
        c.execute('''
            ...
            (1, 'Cotton King', 'Premium Cotton Shirt', 'Fashion', 1000.0, 0, 0.0, '', 'https://images.unsplash.com/photo-1521572267360-ee0c2909d518?w=500&auto=format&fit=crop&q=60', 'Pure breathable formal shirt'),
...
def generate_pdf_invoice(bill_data):
    ...
    pdf.cell(120, 7, "Delivery / Site Logistics Charges")
    pdf.cell(70, 7, f"Rs {bill_data['fee']:,.2f}", ln=True, align="R")
    
    pdf.line(10, 85, 200, 85)
    pdf.ln(3)
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(120, 9, "Total Paid Directly to Store:")
...
if menu == "🛍️ Customer Marketplace":
    ...
    with c2: cust_phone = st.text_input("Customer WhatsApp Phone", value="919876500000")
    ...
    with f1: search_query = st.text_input("🔍 Search any item or property (e.g. Chai, Plot, Shirt, Scorpio, Atta):", "")
    with f2: cat_filter = st.selectbox("Category Filter", ["All Categories", "Real Estate", "Grocery", "Electronics", "Automobile", "Daily Essentials", "Fashion"])
    with f3: price_range = st.selectbox("Budget Filter", ["All Prices (Rs 50 to Rs 50L+)", "Under Rs 500", "Rs 500 - Rs 5,000", "High-Value Deals (Above Rs 50,000)"])
    ...
    query = '''
        SELECT p.id, p.brand, p.title, p.category, p.price, p.is_high_value, p.advance_booking_amount, p.video_url, p.image_url, p.description,
               ...
               v.rating as vendor_rating, v.wallet_balance as vendor_wallet, v.lat, v.lon, v.free_delivery_above_500, v.base_1km, v.base_2km, v.per_km_extra
        FROM products p
    ...
    for _, row in df.iterrows():
        ...
        results.append({
            "p_id": row["id"], "brand": row["brand"], "title": row["title"], "category": row["category"], "price": row["price"],
            "is_high_val": row["is_high_value"], "advance_token": row["advance_booking_amount"], "video_url": row["video_url"],
            "image_url": row["image_url"] if row["image_url"] else "https://via.placeholder.com/300x200?text=Product+Image",
            "desc": row["description"], "v_id": row["vendor_id"], "v_name": row["vendor_name"], "v_phone": str(row["vendor_phone"]),
            "v_upi": row["upi_id"], "v_wallet": row["vendor_wallet"], "v_gstin": row["gstin"], "v_rera": row["rera_id"],
            "v_kyc": row["is_kyc_verified"], "v_sponsored": row["is_sponsored"], "v_rating": row["vendor_rating"],
            "v_lat": row["lat"], "v_lon": row["lon"], "distance": dist, "delivery_fee": fee, "fee_desc": fee_desc
        })
    ...
    if results:
        ...
        for item in results:
            folium.Marker([item["v_lat"], item["v_lon"]], popup=f"{item['v_name']} ({item['distance']} KM)",
                          icon=folium.Icon(color="green" if item["v_sponsored"] == 0 else "purple", icon="shopping-cart")).add_to(m)
            ...
            with cols[idx % 2]:
                with st.container(border=True):
                    ...
                    with info_col:
                        ...
                        st.caption(f"🏬 **Store/Firm:** {item['v_name']} (Direct UPI: `{item['v_upi']}`)")
                    ...
                    if item["v_wallet"] < commission_required:
                        st.error("⚠️ Store temporarily Inactive (Recharge required by shopkeeper).")
                    else:
                        with b_col1:
                            if item["is_high_val"] == 1:
                        ...
                        with b_col2:
                            ...
                            if st.button(btn_label, key=f"btn_{item['p_id']}"):
                                ...
                                cur = conn_o.cursor()
                                cur.execute('''
                                    INSERT INTO orders (customer_name, customer_phone, vendor_id, delivery_otp, items_summary, item_price, amount_paid_now, discount_amount, delivery_fee, grand_total, platform_commission_1pct, vendor_net_payout, distance_km, status)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, 0.0, ?, ?, ?, ?, ?, 'Order Placed')
                                ''', (cust_name, cust_phone, item["v_id"], gen_otp, f"{item['brand']} - {item['title']}", item_total, pay_now, del_fee, grand_total, cut_1pct, grand_total, item["distance"]))
                                
                                # ⚡ Deduct 1% Platform Commission from Store's Prepaid Wallet
                                ...
                                st.session_state.current_bill = {
                                    "order_id": order_id, "cust": cust_name, "cust_phone": cust_phone, "otp": gen_otp,
                                    "item": f"{item['brand']} - {item['title']}", "shop": item["v_name"], "shop_phone": item["v_phone"],
                                    "shop_upi": item["v_upi"], "gstin": item["v_gstin"], "price": pay_now, "discount": 0.0,
                                    "fee": del_fee, "total": grand_total, "cut": cut_1pct, "distance": item["distance"]
                                }

    # Invoice & Direct Merchant QR Section
    if "current_bill" in st.session_state:
        b = st.session_state.current_bill
        st.markdown("---")
        st.success(f"🎉 Order / Booking #{b['order_id']} Placed! 100% Direct Store Payment QR:")
        
        q1, q2 = st.columns([1, 1])
        with q1:
            st.markdown(f"### 📱 Scan to Pay Directly to {b['shop']}")
            qr_bytes = generate_upi_qr(b["shop_upi"], b["shop"], b["total"], f"Order_{b['order_id']}")
            st.image(qr_bytes, width=220)
            st.write(f"**Payee UPI:** `{b['shop_upi']}` | **Total Amount:** :green[**Rs {b['total']:,.2f}**]")
        ...
        with q2:
            ...
            st.info(f"✅ 1% Platform Commission (Rs {b['cut']:,.2f}) auto-deducted from {b['shop']}'s prepaid wallet.")

            pdf_bytes = generate_pdf_invoice(b)
            st.download_button("📄 Download Official PDF Receipt", data=bytes(pdf_bytes), file_name=f"Invoice_Order_{b['order_id']}.pdf", mime="application/pdf")

            msg_text = (f"🛍️ *NEW ORDER / BOOKING #{b['order_id']}*\n👤 Customer: {b['cust']}\n📦 Item: {b['item']}\n💰 Total Paid: Rs {b['total']:,.2f}\n📍 Distance: {b['distance']} KM")
            ...
            if sample_item["v_wallet"] < cut_1pct:
                ...
                if st.button("🚀 Checkout & Pay Directly to Store"):
                    ...
                    cur.execute('''
                        INSERT INTO orders (customer_name, customer_phone, vendor_id, delivery_otp, items_summary, item_price, amount_paid_now, discount_amount, delivery_fee, grand_total, platform_commission_1pct, vendor_net_payout, distance_km, status)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Order Placed')
                    ''', ("Rahul Sharma", "919876500000", sample_item["v_id"], gen_otp, items_summary, items_total, final_total, discount_amount, fee, final_total, cut_1pct, final_total, dist))
                    ...
                    st.session_state.cart = []
                    st.success(f"🎉 Combined Order #{order_id} placed! 1% Commission auto-debited from store wallet.")
                    st.rerun()
        ...
        for _, o_row in my_orders.iterrows():
            with st.container(border=True):
                ...
                with c_t1:
                    ...
                    st.write(f"**Direct Payment Done:** :green[**Rs {o_row['grand_total']:,.2f}**] | Date: `{o_row['created_at']}`")
...
# -----------------------------------------------------------
    ...
    if not vendors_df.empty:
        ...
        st.write("### Recent Orders & 1% Commission Deductions:")
...
# -----------------------------------------------------------
# TAB 8: VENDOR WALLET & REFUND / WITHDRAWAL
# -----------------------------------------------------------
...
# -----------------------------------------------------------
    ...
    t1, t2 = st.tabs(["➕ List New Item / Property", "⚙️ Store Settings & Promotion"])
    with t1:
        with st.form("prod_form"):
            s_id = st.selectbox("Select Store / Enterprise", vendors_df["id"].tolist(), format_func=lambda x: vendors_df[vendors_df["id"] == x]["name"].values[0])
            c_p1, c_p2 = st.columns(2)
            with c_p1:
                b_name = st.text_input("Brand / Project Name", placeholder="e.g. Sai Samruddhi, Tata, Apple")
            ...
            with c_p2:
                p_val = st.number_input("Full Selling Price (Rs)", min_value=50.0, max_value=500000000.0, value=500.0, step=50.0)
        ...
        with st.form("shop_form"):
            ...
            if st.form_submit_button("✅ Register Store / Agency Online"):
                if name and city:
                    ...
                    conn_r.commit()
                    conn_r.close()
                    st.success(f"🎉 '{name}' successfully registered! Recharge wallet to activate.")
...
# -----------------------------------------------------------
    ...
    with an2:
        ...
        st.caption("नोट: 2% (न्यूनतम ₹3) प्रोसेसिंग फीस पहले ही काट ली गई है। आपको बस नीचे लिखी अमाउंट पे करनी है।")
